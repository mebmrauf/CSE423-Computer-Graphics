...
Matt should use the mirror with shininess = 20. It provides the optimal balance - sharp enough
for clear enemy detection but not so focused that it misses the target if positioning isn't perfect.
```
...
while the diffuse reflections maintain the car's red paint color. In Phong's model, specular highlights
take on the light source color regardless of the object's base color.
```
